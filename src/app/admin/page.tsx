...
export default function TabsDemo() {
  ...
  return (
    <div className="xl:px-10 px-4 ">
      <Tabs defaultValue="managerProduct" className="  gap-2  grid grid-cols-1">
        {isLargeScreen ? (
          <TabsList
        ...
        flex justify-between items-center flex-row   max-xl:flex-wrap "
            >
              <TabsTrigger value={'home'} className="xl:min-w-[200px] flex justify-start items-center gap-3 max-xl:text-sm">
                ...
                <DropdownMenuContent className="w-56 bg-slate-300 text-black p-2 rounded-lg relative z-[9999]">
                  <DropdownMenuGroup>
                    {MenuAdmin.map((item, index) => {
                      ...
                      return (
                        <TabsTrigger key={index} value={item.value} className="w-full flex justify-start items-center gap-3 max-xl:text-sm">
                          {item.icon}
                          {item.title}
                        </TabsTrigger>
                      );
