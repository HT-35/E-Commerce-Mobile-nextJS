...
export function AcceptDeleteProduct({
  ...
  const handleDeleteProduct = async (slug: string, name: string) => {
    ...
    if (deleteProduct.statusCode === 200) {
      setProduct((prevProducts: any[]) => prevProducts.filter((item) => item.slug !== slug));
  ...
  return (
    <AlertDialog open={acceptDelete}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Bạn có chắc chắn muốn xóa sản phẩm {dataDeleteProdcut.name}</AlertDialogTitle>
          <AlertDialogDescription>Sau khi xóa sản phẩm sẽ không thể khôi phục.</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel onClick={() => setAcceptDelete(false)}>Hủy</AlertDialogCancel>
          <AlertDialogAction className="relative" onClick={() => handleDeleteProduct(dataDeleteProdcut.slug, dataDeleteProdcut.name)}>
            Xác Nhận Xóa
            {loadding && (
